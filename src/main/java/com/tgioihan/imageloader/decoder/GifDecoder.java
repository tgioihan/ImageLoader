...
public class GifDecoder {
    ...
    public long getSize(){
        ...
        if(suffix!=null)
            total+=suffix.length;
        if(pixelStack!=null)
            total+=pixelStack.length;
        if(mainPixels!=null)
            total+=mainPixels.length;
        if(convertBitmap!=null&&!convertBitmap.isRecycled())
            total+= ImageSizeUtil.getBitmapSize(convertBitmap);

        if(previousImage!=null&&!previousImage.isRecycled())
            total+= ImageSizeUtil.getBitmapSize(previousImage);
        return total;
    }
    ...
    public Bitmap getNextBitmap() {
        ...
        if(convertBitmap==null){
            while (!done){
                try{
                    convertBitmap = Bitmap.createBitmap(header.width, header.height, BITMAP_CONFIG);
                    done = true;
                }catch (OutOfMemoryError outOfMemoryError){
                    outOfMemoryError.printStackTrace();
                    header.scalDown(sampleSize*2);
                    done = false;
                }
            }
        }
        setAlpha(convertBitmap);
